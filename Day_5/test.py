...
def cheak_range(row,intermediate_key):
  if row[1]<=intermediate_key and intermediate_key<row[1]+row[2]:
    return row[0]+(intermediate_key-row[1])
  return -1

def key_to_location(total_input,seed):
  intermediate_key = seed
  for set in total_input:
    for row in set:
      intermediate_value = cheak_range(row,intermediate_key)
...
location=[]
for seed in keys:
  location.append(key_to_location(total_input,seed))
